...
Изучите разницу между **Function expression**, **Function Declaration** и  **Arrow function**
...
```
Заготовки для функций вы можете найти [тут](./src//chapter_3/counter.js)
...
Также надо написать 4 функции основных арифмитических операторов, которые принимают в кач-ве аргумента первое число, а возвращают функцию, которая принмает в качестве аргумента второе число и возвращает их сумму/разность/частое/произведение
Результат:
```
one(mult(three(plus(four())))); // В итоге вернется 7
```
Заготовки для функций вы можете найти [тут](./src//chapter_3/calculator.js)
<br>
...
paskalsTriangle(3, 2) // В итоге вернется 3
```
Заготовки для функций вы можете найти [тут](./src//chapter_3/paskalsTriangle.js)
<br>
...
fibo(6) // Вернет 8 
```
Заготовки для функций вы можете найти [тут](./src//chapter_3/fibonachi.js)
...
<br>
**Задание 4.**  Вам нужно написать функцию которая принимает в кач-ве аргумента массива чисел и удаляет все повторяющиеся значения.
...
```
Заготовки для функций вы можете найти [тут](./src//chapter_4/removeReps.js)
...
**Задание 5.** Напишите функцию, которая на вход принимает две строки - сообщение (обычная строка с текстом) и символ который надо удалить из этого сообщения
Обязательно воспользуйтесь методами и циклами, которые вы изучили.
```
function removeString(message, symbol) {}

removeString("Большое и интересное сообщение", "о"); // Бльше и интересне сбщение
```
Заготовки для функций вы можете найти [тут](./src//chapter_4/removeSymbol.js)

<br>

**Задание 6.** Напишите функцию банкомат которая принимает на вход число и возвращает объект в формате  {номинал_купюры : количество_купюр}.
...
```
Заготовки для функций вы можете найти [тут](./src//chapter_4/atm.js)

<br>
**Задание 7.** В этой задаче нужно будет написать алгоритм поиска, который скажет, можно ли найти входное слово в головоломке поиска слов, которая тоже подается функции на вход.
...
Пример головоломки и результат вывода можно найти [тут](./src//chapter_4/sudoku.js)

<br>
И финальное задание на сегодня, здесь вам понадобятся знание полученные в ходе изучения тем и выполенения предыдущих заданий. Готовы?)
**Задание 8.** Вам нужно написать функция для решения Судоку. Функция принимает один аргумент - строку, в ней на месте пропущенных цифр стоят -.
...
6 - - 3 - 8 9 - -
```
[В файле задания](./src//chapter_4/) представленны тестовые данные для вашей функции, они имеют 3 уровня сложности
